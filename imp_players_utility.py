...
def prepare_repr_text_ftrs(teams_dict, players_dict, embedding_model):
    player_ftrs = [' <PLAYER> '.join([f"{key.upper()} {val}" for key, val in item.items()]) for item in players_dict]
    team_ftrs = [' <TEAM> '.join([f"{key.upper()} {val}" for key, val in item.items()]) for item in teams_dict]
    game_ftrs = f"<PLAYER> {player_ftrs} <TEAM> {team_ftrs}"
    return embedding_model.encode([game_ftrs])[0]

def prepare_repr_num_ftrs(teams_dict, players_dict):
    player_ftrs = np.array([list(i.values()) for i in players_dict[:10]]).ravel()
    team_ftrs = np.array([list(i.values()) for i in teams_dict]).ravel()
    # game_ftrs = np.append(player_ftrs, team_ftrs)
    game_ftrs = player_ftrs
    return game_ftrs
...
def prepare_repr_set_ftrs(teams_dict, players_dict):
    ...
    game_ftrs = list(player_ftrs.values())
    return np.array(game_ftrs)

def get_game_repr_imp_players(score_dict, ger_obj, imp_players, embedding_model, ftrs_type='cosine'):
    """
    ...
    if len(player_ftrs) < 10:
        player_ftrs.extend([ger_obj.get_empty_bs_dict(winner=winner) for _ in range(10 - len(player_ftrs))])
